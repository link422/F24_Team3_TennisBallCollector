...
According to the Bill of Materials, there were 3 possible solutions examined for this subsystem, all of which use components and materials that comply with the Restriction of Hazardous Substances standard. The purpose of these solutions is to provide a guideline to construct the subsystem to work with the specifications and constraints given for the project and this subsystem. Solution number one is to alter the component being used from the EA DOGM132L-5 [6] to the EA DOGM162W-A [3]. This solution includes the implementation from a potential source code [11] for the Counting Display Subsystem. One reason not to use this solution is the difficulty of implementation because this solution involves modifing the driver to interface from the raspberry pi 4 model B [1] to the DOGM162W-A [2] rather than writing directly to the DOGM162W-A [2]. 
The second solution has an easier integration but costs more. This involves soldering a I2C LCD adapter to the LCD display [4], connecting the component to a I2C logic level converter [5], then connecting the I2C logic level converter [5] to gpio pins of the subsystem. This solution involves simplier coding integration, see [12], with other subsystems including the Counting Sensor, and RC subsystems. 
The final solution is to keep the LCD [6] unchanged from the conceptual design and rewriting the driver with more points for error in the design. The solutions listed in the detailed design were ordered based on a combination of cost and ease of integration with the project design and other subsystems.   
...
[12] "Control a HD44780 LCD display via I2C with the Raspberry Pi" <https://tutorials-raspberrypi.com/control-a-raspberry-pi-hd44780-lcd-display-via-i2c/> (accessed Nov. 23, 2024).
